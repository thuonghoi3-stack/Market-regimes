...
// Same-origin path, proxied server-side to https://grok.com/grok-app-builder/extensions.js
// by server/middleware/grok-extensions-proxy.ts. Deployed responses carry
// `Cross-Origin-Embedder-Policy: require-corp` (vercel.json, required by the
// pglite/WASM setup in src/lib/db.ts), which makes the browser block any
// cross-origin <script src> that lacks a matching Cross-Origin-Resource-Policy
// header. grok.com does not send one for this asset, so pointing straight at
// it here reliably fails to load on the deployed app — loading it through our
// own origin keeps it same-origin (COEP never applies) while still serving
// the identical grok.com file and banner behavior.
export const GROK_EXTENSIONS_SCRIPT_SRC = "/__grok/extensions.js";
...
export function injectGrokPwaHead(html, ctx = {}) {
  ...
  if (!next.includes(GROK_EXTENSIONS_SCRIPT_SRC)) {
